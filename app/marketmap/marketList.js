import { Group, Avatar, Text, Accordion, AccordionItem, AccordionControl, AccordionPanel, Container, Button, Flex } from '@mantine/core';
...
export default function MarketAccordion() {
    const items = charactersList.map((item) => (<AccordionItem value={item.id} key={item.label}>
        ...
        <AccordionPanel>
            ...
            <Button component={Link} target="_blank" href={item.link}>Directions</Button>
            </Flex>
        </AccordionPanel>
        </AccordionItem>));
        return (
